...
async def test_completed_stage_is_idempotent():
    settings = AsyncMock(retrospective_enabled=True, retrospective_max_items=20)
    state = {"ticket_key": "F-1", "current_node": "complete"}
    key = build_input(state).idempotency_key
    with patch("forge.workflow.retrospective.JiraClient") as jira:
        result = await run_retrospective(state | {"retrospective_keys": [key]}, settings)
...
@pytest.mark.parametrize(
    "state",
    [
        {"current_node": "human_review_gate", "is_paused": True},
        {"current_node": "plan_approval_gate", "is_paused": True},
        {"current_node": "complete", "is_paused": True},
    ],
)
def test_waiting_states_are_not_terminal(state):
    assert terminal_outcome(state) is None


def test_retry_generation_gets_a_distinct_key():
    blocked = build_input({"ticket_key": "F-1", "is_blocked": True, "retry_count": 0})
    retried = build_input({"ticket_key": "F-1", "current_node": "complete", "retry_count": 1})
    assert blocked.idempotency_key != retried.idempotency_key

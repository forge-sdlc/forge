...
class TestGraphCompilation:
    ...
    def test_ci_fix_routes_to_human_review_gate(self):
        """attempt_ci_fix can route to human_review_gate after fixing."""
        graph = build_bug_graph()
        compiled = graph.compile()
        targets = {e.target for e in compiled.get_graph().edges if e.source == "attempt_ci_fix"}

        assert "human_review_gate" in targets

    def test_review_fix_routes_to_human_review_gate(self):
        """implement_review can route to human_review_gate after implementing changes."""
        graph = build_bug_graph()
        compiled = graph.compile()

...
from forge.workspace.output_validation import ChangedEntry, OutputValidationContext
...
def test_agent_output_uses_explicit_provenance() -> None:
    with pytest.raises(SecretDetectedError) as caught:
        scan_agent_output(
            f"aws_access_key_id = {AWS_KEY}",
            context=AgentOutputContext(
                source="PR body", ticket_key="FORGE-1", workflow_stage="pr_creation"
            ),
        )

    assert caught.value.findings[0].location == "PR body (pr_creation)"
...
def test_repository_validator_uses_common_gate_context(repository: Path) -> None:
    (repository / "generated.env").write_text(f"aws_access_key_id={AWS_KEY}\n")
    _git(repository, "add", "generated.env")
    _git(repository, "commit", "-m", "generated")
    base = subprocess.run(
    ...
    # The gate must inspect the exact pushed tree, not a potentially different worktree.
    (repository / "generated.env").write_text("safe=true\n")

    with pytest.raises(SecretDetectedError):
        RepositorySecretValidator().validate(
            OutputValidationContext(
                repo_path=repository,
                base_ref=base,
                head_ref=head,
                changed_entries=(
                    ChangedEntry("A", "100644", "generated.env", len(AWS_KEY) + 19),
                ),
            )
        )

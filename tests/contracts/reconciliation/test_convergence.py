...
async def test_lost_duplicate_stale_and_reordered_delivery_converges() -> None:
    ...
    # and the old opened revision is delivered after it. A merge is deliberately
    # a separate terminal resource, so the historical open observation remains
    # accepted but cannot reopen or otherwise affect the merged PR lifecycle.
    ...
    assert degraded["delivery_dispositions"] == ["accepted", "duplicate", "accepted", "duplicate"]

...
from forge.integrations.source_control.contracts import EventKind, NormalizedEvent
...
def normalized_event_to_observation(
    ...
    *,
    ...
    if event.check:
        ...
        if event.kind is EventKind.CR_MERGED:
            # A merged pull request is terminal.  It must not be compared to
            # mutable PR head revisions or check observations: GitHub may
            # deliver it after those observations and its head SHA is not a
            # condition of completion.  The resource is intentionally scoped
            # to the repository and PR number, with a stable terminal revision
            # so repeated merge deliveries reconcile idempotently.
            resource_type = "change_request_merge"
            revision = "merged"
        else:
            resource_type = "change_request"
            revision = change_request.head_sha or None

    if event.kind is EventKind.CR_MERGED and change_request:
        # Keep terminal deliveries idempotent even if GitHub supplies a
        # different head SHA or actor representation on a redelivery.  The
        # normalized event itself remains available to the transition layer.
        facts = _json_value(
            {
                "kind": event.kind,
                "repository": event.repo_ref,
                "change_request": {
                    "identity": change_request.identity,
